import { ATTRIBUTE_LABELS, attributeGroupId, type AttributeKey } from "../lib/attributes";
...
export function StatIcon({ stat, value, className }: Props) {
  ...
  const group = attributeGroupId(stat);
  return (
    <span
      className={`stat-icon stat-icon--${group}${className ? ` ${className}` : ""}`}
      title={label}
      aria-label={label}
    >
